...
from app.intelligence.history_intelligence import HistoryIntelligence
...
history_engine = HistoryIntelligence()
...
def specialist_parallel_node(state: MeetingState) -> MeetingState:
    ...
    def run_engineering():
        ...
        result = engineering_agent.analyze(
            transcript,
        )
...
def history_node(state: MeetingState) -> MeetingState:
    """
    AI Memory Layer

    Compares current meeting against previous meetings
    stored in MongoDB.
    ...
    previous_meetings = meeting_repository.get_recent(
        limit=5,
    )

    current_meeting = {
        "meeting_health_score": state.get(
            "meeting_health_score",
            0,
        ),
        "ml_risk_probability": state.get(
            "ml_risk_probability",
            0,
        ),
        "risks": state.get(
        ...
        "engineering_risks": state.get(
            "engineering_risks",
            [],
        ),
        "engineering_recommendations": state.get(
            "engineering_recommendations",
            [],
        ),
        "finance_risks": state.get(
            "finance_risks",
            [],
        ),
        "finance_recommendations": state.get(
            "finance_recommendations",
            [],
        ),
        "risk_insights": state.get(
            "risk_insights",
            [],
        ),
    }

    history = history_engine.analyze(
...
def coordinator_node(state: MeetingState) -> MeetingState:
    ...
    result = coordinator_agent.analyze(
        state,
    )
